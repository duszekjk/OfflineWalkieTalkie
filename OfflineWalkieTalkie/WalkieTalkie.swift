...
final class WalkieTalkie: NSObject, ObservableObject, CLLocationManagerDelegate {
    ...
    private var remoteNamesByID: [String: String] = [:]
    ...
    override init() {
        ...
        if let data = UserDefaults.standard.data(forKey: "localLocations"),
           let locations = try? JSONDecoder().decode([LocationPoint].self, from: data) {
            localLocations = locations
        }
        if let data = UserDefaults.standard.data(forKey: "remoteDevices"),
           let devices = try? JSONDecoder().decode([ConnectedDevice].self, from: data) {
                ...
                if !self.localAudioActive,
                   ...
                   self.sentLocationCount < self.localLocations.count {
                    ...
                    let update = LocationUpdate(
                        id: self.deviceID,
                        points: Array(self.localLocations[self.sentLocationCount..<end])
                    )
    ...
    private func persistHistory() {
        if let data = try? JSONEncoder().encode(localLocations) {
            UserDefaults.standard.set(data, forKey: "localLocations")
        }
    ...
    private func updateCapture() {
        let shouldCapture = connected &&
            microphoneReady &&
            ((mode == .walkieTalkie && isTalking) ||
             (mode == .call && callActive && !microphoneMuted))
        ...
        input.installTap(
            ...
            if !self.localAudioActive {
                ...
                for buffered in self.preRoll {
                    self.sendAudio(buffered, through: connection)
                }
    ...
    private func resetConnection() {
        ...
        browser?.cancel()
        browser = nil
        remoteNamesByID.removeAll(keepingCapacity: true)
        sentLocationCount = 0
                ...
                if let data {
                    ...
                    while self.receivedData.count >= 2 {
                        ...
                        if type == 0 {
                            ...
                            let key = self.normalizedName(identity.name)
                            self.remoteNamesByID[identity.id] = key

                            if let index = self.remoteDevices.firstIndex(where: { $0.id == key }) {
                            ...
                            let key = self.remoteNamesByID[update.id] ?? update.id
                            if let index = self.remoteDevices.firstIndex(where: { $0.id == key }) {
                                let known = Set(self.remoteDevices[index].locations.map(\.id))
                                self.remoteDevices[index].locations.append(
                                    contentsOf: update.points.filter { !known.contains($0.id) }
                                )
                                if self.remoteDevices[index].locations.count > 300 {
                                    self.remoteDevices[index].locations.removeFirst(
                                        self.remoteDevices[index].locations.count - 300
                                    )
                                }
                                self.persistHistory()
                            }
